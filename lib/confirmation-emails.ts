...
export function buildOrderHtml(p: OrderPayload): string {
...
export function buildRegistrationHtml(p: RegistrationPayload): string {
  const firstName = p.customerName.trim().split(' ')[0] || p.customerName
...
// ── Send helpers with failure logging ──────────────────────────────────────────

/**
 * Resend does not throw on API errors — it returns { error }. Failures were
 * previously silenced; log them in a structured way and, when the email_log
 * table exists, persist them for monitoring.
 */
async function logEmailFailure(kind: string, to: string, subject: string, error: unknown) {
  const message = error instanceof Error ? error.message : JSON.stringify(error)
  console.error('[email-failure]', JSON.stringify({ kind, to, subject, error: message }))
  try {
    ...
    // email_log table not present yet — the console.error above still records it
  }
}

export async function sendOrderConfirmation(p: OrderPayload): Promise<boolean> {
  const resend = new Resend(process.env.RESEND_API_KEY)
  const subject = 'Order Confirmed — Alata Investment Club'
  try {
    const { error } = await resend.emails.send({
      from: FROM,
      to: p.customerEmail,
      subject,
      html: buildOrderHtml(p),
    })
    if (error) {
      await logEmailFailure('order-confirmation', p.customerEmail, subject, error)
      return false
    }
    return true
  } catch (e) {
    await logEmailFailure('order-confirmation', p.customerEmail, subject, e)
    return false
  }
}

export async function sendRegistrationConfirmation(p: RegistrationPayload): Promise<boolean> {
  const resend = new Resend(process.env.RESEND_API_KEY)
  const eventName = p.events[0]?.name ?? 'Event'
  ...
  try {
    const { error } = await resend.emails.send({
      ...
      to: p.customerEmail,
      subject,
      html: buildRegistrationHtml(p),
    })
    if (error) {
      await logEmailFailure('registration-confirmation', p.customerEmail, subject, error)
      return false
    }
    return true
  } catch (e) {
    await logEmailFailure('registration-confirmation', p.customerEmail, subject, e)
    return false
  }
}
